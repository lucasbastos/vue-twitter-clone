<template>
      ...
      <div>
        ...
        <div>
          <button v-for="(tab, index) in tabs" @click="id = tab.id" :class="`focus:outline-none hover:text-blue flex items-center py-2 px-4 hover:bg-lightblue rounded-full mr-auto mb-3 ${ id === tab.id ? 'text-blue' : ''}`" :key="index">
      ...
      <div class="lg:w-full relative">
        <button @click="dropdown = true" class="flex items-center w-full hover:bg-lightblue rounded-full p-2 focus:outline-none">
          <img src="https://pbs.twimg.com/profile_images/1311144185509146627/ekPPg1I3_400x400.jpg" class="w-10 h-10 rounded-full border border-lighter" />
          <div class="hidden lg:block ml-4">
            <p class="text-sm font-bold leading-tight"> Lucas Bastos </p>
            <p class="text-sm leading-tight"> @lucasbastos.dev </p>
          </div>
    ...
    data() {
        return {
            tabs: [
                ...
                {icon: 'far fa-bell', title: 'Notificações', id: 'notifications'},
                {icon: 'far fa-envelope', title: 'Mensagens', id: 'messages'},
                {icon: 'far fa-bookmark', title: 'Itens salvos', id: 'bookmarks'},
                {icon: 'fas fa-clipboard-list', title: 'Listas', id: 'lists'},
                {icon: 'far fa-user', title: 'Perfil', id: 'profile'},
